...
function BookstoreSVG() {
  return (
    <svg viewBox="0 0 480 120" width="100%" height="100%" preserveAspectRatio="xMidYMid slice">
      ...
      <text x="10" y="115" fill="#1c1c1c" fontFamily="'Courier New', monospace" fontSize="7" fontWeight="bold">
...
function SneakerSVG() {
  return (
    <svg viewBox="0 0 240 120" width="100%" height="100%" preserveAspectRatio="xMidYMid slice">
      ...
      {/* Laces */}
      {[90, 110, 130].map((x, i) => (
        <line key={i} x1={x} y1="70" x2={x + 20} y2="72"
          stroke={GOLD_BRIGHT} strokeWidth="0.8" opacity="0.5" />
      ))}
      {/* Stars */}
...
function FashionSVG() {
  return (
    <svg viewBox="0 0 240 120" width="100%" height="100%" preserveAspectRatio="xMidYMid slice">
      ...
      {/* Clothing body fill */}
      <path d="M85,55 L75,90 L165,90 L155,55 Q140,48 120,46 Q100,48 85,55Z"
        fill="#141414" />
...
function StatusBadge({ status }) {
  const config = {
    ...
    certified:  { label: '✦ CERTIFIED', color: GOLD,        bg: 'rgba(201,150,58,0.1)', border: 'rgba(201,150,58,0.3)' },
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ fontSize: '12px', color: SILVER, lineHeight: 1.7, maxWidth: '600px', fontFamily: "'Courier New', monospace", opacity: 0.7 }}>
            {project.expandDesc}
          </p>
        </div>
        <button
          ...
          onMouseLeave={(e) => { e.target.style.borderColor = 'rgba(201,150,58,0.2)'; e.target.style.color = SILVER; }}
        >
          ✕ Close
        </button>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
        {project.metrics.map((m, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontSize: '22px', fontWeight: 700, color: GOLD_GLOW, fontFamily: "'Courier New', monospace", marginBottom: '4px' }}>
              {m.value}
            </div>
            <div style={{ fontSize: '8px', color: '#555', fontFamily: "'Courier New', monospace", textTransform: 'uppercase', letterSpacing: '0.15em' }}>
              {m.label}
            </div>
          </div>
        ))}
  ...
  return (
    <motion.div
      ...
      style={{
        ...motionStyle,
        gridColumn: 'span 1',
        background:  hovered ? '#0E0E12' : '#09090D',
        border:      hovered ? `1px solid rgba(201,150,58,0.3)` : '1px solid rgba(201,150,58,0.08)',
        boxShadow:   hovered ? `0 12px 30px -10px rgba(201,150,58,0.15)` : 'none',
        position: 'relative',
        cursor: 'pointer',
        overflow: 'hidden',
        padding: '22px 20px',
        ...
        zIndex: 10,
        ...(isMobileSlider ? { width: '78vw', maxWidth: '300px', flexShrink: 0, height: '370px' } : {}),
      }}
      onClick={onToggle}
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        <span style={{ fontFamily: "'Courier New', monospace", fontSize: '8px', color: '#444', textTransform: 'uppercase', letterSpacing: '0.15em' }}>
          {project.tag}
        </span>
      ...
      <div style={{ fontFamily: "'Courier New', monospace", fontSize: '8px', color: '#2a2a2a', marginBottom: '6px' }}>
        {project.num} / {project.category.toUpperCase()}
      </div>

      <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#F0F0EE', marginBottom: '8px', fontFamily: "'Inter', sans-serif" }}>
        {project.title}
      </h3>

      <p style={{ fontSize: '11px', color: '#555', lineHeight: 1.6, marginBottom: '16px', fontFamily: "'Courier New', monospace", flexGrow: 1 }}>
        {project.desc}
      </p>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #141414', paddingTop: '12px' }}>
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          {project.tech.map((t, i) => (
            <span key={i} style={{
              fontFamily: "'Courier New', monospace", fontSize: '8px',
              color: hovered ? GOLD : '#333',
              border: `1px solid ${hovered ? 'rgba(201,150,58,0.25)' : '#1a1a1a'}`,
...
function MobileProjectSlider({ projects }) {
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <p style={{ textAlign: 'center', fontFamily: "'Courier New', monospace", fontSize: '9px', color: '#2a2a2a', letterSpacing: '0.15em', marginTop: '10px', textTransform: 'uppercase' }}>
        ← swipe to browse →
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage: 'linear-gradient(rgba(201,150,58,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(201,150,58,0.02) 1px, transparent 1px)',
      ...
      <motion.div
        style={{
          ...
          boxShadow: `0 0 20px 6px rgba(201,150,58,0.15), 0 0 6px 1px ${GOLD}`,
          zIndex: 5,
          pointerEvents: 'none',
        }}
      />
      ...
      >
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
            <div>
              <span style={{ fontFamily: "'Courier New', monospace", fontSize: '10px', fontWeight: 700, letterSpacing: '0.4em', textTransform: 'uppercase', color: GOLD, display: 'block', marginBottom: '10px' }}>
                SELECTED PROJECTS
              </span>
              <h2 style={{ fontFamily: "'Anton', sans-serif", fontSize: 'clamp(36px, 7vw, 80px)', lineHeight: 1, textTransform: 'uppercase', letterSpacing: '-0.01em', color: '#F8F8F6', margin: 0 }}>
                SELECTED WORK
              </h2>
            </div>
            <span style={{ fontFamily: "'Courier New', monospace", fontSize: '10px', color: '#3a3a3a', letterSpacing: '0.1em' }}>
              {TABS.find(t => t.key === activeProjectTab)?.label || 'All Work'} / {count}
            </span>
          </div>

          {/* Filter Tabs */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '24px', paddingBottom: '20px', borderBottom: '1px solid rgba(201,150,58,0.08)', overflowX: isMobile ? 'auto' : 'visible' }}>
            {TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '100px',
                    border: `1px solid ${isActive ? GOLD : 'rgba(201,150,58,0.12)'}`,
                    background: isActive ? 'rgba(201,150,58,0.12)' : 'transparent',
                    color: isActive ? GOLD_BRIGHT : '#555',
                    cursor: 'pointer',
          ...
          {isMobile ? (
            ...
            <>
              {/* Desktop Grid */}
              <div className="frey-project-grid" style={{ display: 'grid', gap: '1px', background: 'rgba(201,150,58,0.04)' }}>
                {buildGrid()}
              </div>

              {/* Pagination */}
              {totalPages > 1 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '40px', paddingTop: '20px', fontFamily: "'Courier New', monospace" }}>
                  <span style={{ fontSize: '10px', color: '#3a3a3a', letterSpacing: '0.1em' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <button
                      ...
                      style={{
                        background: 'none',
                        border: `1px solid ${currentPage === 1 ? '#1a1a1a' : 'rgba(201,150,58,0.3)'}`,
                        color: currentPage === 1 ? '#2a2a2a' : GOLD,
                    ...
                    <button
                      ...
                      style={{
                        background: 'none',
                        border: `1px solid ${currentPage === totalPages ? '#1a1a1a' : 'rgba(201,150,58,0.3)'}`,
                        color: currentPage === totalPages ? '#2a2a2a' : GOLD,
